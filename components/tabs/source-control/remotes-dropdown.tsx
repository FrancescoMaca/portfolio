...
export function RemotesDropdown({ expanded }: { expanded: boolean }) {
  ...
  return (
    <div className={`flex flex-col overflow-hidden ${expanded ? 'h-full' : 'h-0'} transition-all duration-500 cursor-pointer`}>
      <div className="flex px-5 py-1 gap-2 text-text-normal hover:bg-hover-dark"
        onClick={() => isExpanded(!subExpanded)}
      >
        <Image src={`/svg/ide/chevron-right.svg`} alt="" width={20} height={20}
          className={`min-w-[20px] ${subExpanded ? 'rotate-90' : ''} transition-all`}
        />
        <Image src={userInfo?.avatar_url || '/pictures/def-github.png'} alt="Avatar User" title="" width={20} height={20}
      ...
      <div className={`transition-all overflow-hidden ${subExpanded ? 'h-full' : 'h-0'}`}>
        {
          branches.map((branch, index) =>
            ...
            `}>
              ...
              <span>{formatRelativeTime(new Date(repoInfo?.updated_at || Date.now()))}</span>
            </div>
          )
        }
      </div>
